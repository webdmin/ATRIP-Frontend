...
const SubscriptionPage = () => {
  ...
  const [userEmail] = useState(""); // State for user email
...
const webpageReachData = [5, 12, 10, 35, 55];   
  ...
  const handleLogout = () => {
    ...
    window.location.reload(); // Optionally refresh the page or redirect to login page
  };
      ...
      <div className={`main-content ${isChatbotPage ? 'no-padding' : ''}`}>
        {selectedOption === "Overview" && (
          <header className="header2">
            <h1>Welcome, {isAdmin() ? "Admin" : userEmail.split("@")[0]}</h1> {/* Welcome message */}
          </header>
        )}
