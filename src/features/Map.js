...
import '../features/Map.module.css'
...
const MapWithRoutesAndChatbot = () => {
    ...
    try {
      ...
      const details = {
        ...
        roadInfo: routeDetails.legs[0].steps.map(step => ({
          ...
          //trafficFlow: step.traffic_speed || 'N/A', // Traffic speed (if available, based on some API setup)
          cycleFlow: step.bike_lanes || 'N/A', // Cycling lanes info (if available)
          laneWidth: step.lane_width || 'N/A', // Lane width (if available)
          trafficFlow: routeDetails.legs[0].duration_in_traffic ? `${routeDetails.legs[0].duration_in_traffic} with traffic` : 'No traffic data', // Traffic information (if available)
          cycleFriendly: routeDetails.legs[0].steps.some(step => step.bike_lanes) ? 'Bike lanes available' : 'No bike lanes', // Check if there are bike lanes
          roadNames: routeDetails.legs[0].steps.map(step => step.maneuver.instruction).join(' -> ') // All road names from instructions
        }))
      };
  ...
  return (
    <div style={{ display: 'flex', height: '82vh' }}>
      {/* Map section (75%) */}
      <div style={{ flex: '0 0 75%', height: '82vh', zIndex: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '20px', color: '#fff' }}>
          <h3 style={{ color: 'black', marginTop:'14px', fontFamily:'sans-serif' }}>Active Travel Resilient Infrastructure Planner</h3>
        </div>
        <div style={{
          marginBottom: '20px',
